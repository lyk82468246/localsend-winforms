...
3. HTTP 路由和文件流不依赖某个 TLS 实现。HTTP 服务端接收一个普通 `Stream`，客户端也只在建立 TCP 后把 `Stream` 交给 TLS provider。
...
RuntimeEnvironmentInfo  --->  TlsProviderRouter  --->  ITlsProvider
        ...
        |
LocalSendService ---> HttpServer(Stream) <--- SimpleHttpClient(Stream)
        |                    |
        +-- v1/v2 handlers   +-- plain NetworkStream or TLS-wrapped Stream
```
...
- `Full`：TLS 1.2、服务端和客户端路径以及双向证书自检均通过。

`SupportsStreamTransport` 是一个独立维度。Positron ABI 2 当前拥有自己的 listener/socket，因而它可以报告 ABI/身份能力但不能直接包装本项目已经建立的 `NetworkStream`。在这个状态下 LocalSend 不会把 HTTP 端口错误地宣称为 HTTPS；About 会同时显示 capability 和 “HTTP TLS transport: not connected”。
...
4. 捕获 `DllNotFoundException`、`EntryPointNotFoundException` 和其他异常，转换为 `tls.positronDllMissing`、`tls.positronAbiInvalid` 等 reason code。

正式部署时 DLL 应放在 exe 同目录或 Windows 可搜索目录，且必须是目标 ARMv4/VC 架构。PC 进程不会加载 ARM DLL；PC 一律走 Schannel。

Positron ABI 2 的 listener/socket 生命周期由 DLL 持有，故本项目暂不把它伪装成 `Stream`。未来若要在 CE 真正宣告 HTTPS 服务，需要增加一个明确的 `IPositronSocketTransport` 适配器，把 ABI 的 accept/connect/read/write/close 映射为本项目的 HTTP 服务；该适配器完成前，CE 安全降级到 HTTP 是预期行为。

## HTTP 与官方 v2.2

本项目没有把 HTTP 迁移到 Positron。`HttpServer` 仍是一个小型 `TcpListener` HTTP/1.1 实现，`SimpleHttpClient` 负责 PC/CF 共有的 Content-Length 请求；二者只通过 `Stream` 与 TLS 解耦。
...
降级只发生在 provider 探测报告为 `Unavailable`，或 Positron socket transport 尚未接入时。此时：
